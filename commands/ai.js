...
async function callAIAPIWithTools(prompt, model, apiKey, apiEndpoint, roomJid, imageBuffer = null, imageType = null, userMessage = null, group = null, workingIdentifier = null, pdfFilePath = null) {
    ...
    while (response.stop_reason === 'tool_use' && iterations < MAX_ITERATIONS) {
            ...
            try {
                ...
                if (parsed.filePath && parsed.success && parsed.type === 'image') {
                    ...
                    if (parsed.includeForAnalysis === true) {
                        console.log(`[AI] AI requested image inclusion for analysis`);

                        // Read the file and prepare for inclusion
                        const fs = require('fs');
                        ...
                        console.log(`[AI] File path will be used by other tools (no analysis requested)`);
                    }
                }
            } catch (e) {
                // Not JSON, skip
        ...
        // If analysis was detected, include the downloaded cache image in payload
        ...
        // Next API call with tool results (and image if analysis detected)
        response = await callAIAPI(messages, toolDefinitions, systemPrompt, model, apiKey, apiEndpoint);
    }
